...
def photo(id):
    ...
    # return render_template('photo.html', title="Info", logged_in=logged_in,
    # info_of_image=info_of_image, data_for_image=data_for_image,
    # tags_of_image=tags_of_image, list_of_tags=list_of_tags,
    # format_of_image=format_of_image, form=form)
        ...
        if form.validate_on_submit():
            ...
            if check_duplicate_photo is None:
                ...
                if new_location != '':
                    ...
                    check_location_duplicate = models.Locations.query.filter_by(
                        location_name=new_location).all()
                    if check_location_duplicate == []:
                        add_location = models.Locations(
                            location_name=new_location)
                        db.session.add(add_location)
                        db.session.commit()
                    else:
                        flash("New location added is a dupliacte try again!")
                    find_location_id = models.Locations.query.filter_by(
                        location_name=new_location).all()
